...
# This function initiates and creates the system and role conversation with Open AI model
def initialize_conversation():
    
    '''
    Initializes a system message for OpenAI conversation.
...
def get_chat_model_completions(messages):
    """
    Fetches a response from the OpenAI chat model based on the provided messages.
    """
    try:
        ...
        print(f"Error in fetching completions: {e}")
        return None
    


# Checks user input for potentially harmful content.
def moderation_check(user_input):
    """
    Checks if the user input contains any harmful content using OpenAI's moderation tool.
    """
    try:
        response = openai.moderations.create(input=user_input)
        moderation_output = response.results[0].flagged
        return "Flagged" if moderation_output else "Not Flagged"
    except Exception as e:
        print(f"Error in moderation check: {e}")
        return "Error"

# Intent Confirmation Layer (for evaluating user profile)
# The intent confirmation layer evaluates the output of the chat completion to ensure the user's intent has been captured.
def intent_confirmation_layer(response_assistant):
    """
    Confirms whether the assistant correctly captured the user's laptop requirements.
    """
    delimiter = "####"
    prompt = f"""
    ...
    Remember to return No if any one of the values is not captured
...
# Extracts the user's requirements from the assistant's response and formats them into a string.
def get_user_requirement_string(response_assistant):
    """
    Extracts the formatted user requirement string from the assistant's response.
    """
    delimiter = "####"
    prompt = f"""
...
# Custom function for using Open AI function calling
...
# Calls the OpenAI API with function calling to extract the user's laptop requirements.
def get_chat_completions_func_calling(input, include_budget):
    """
    Uses OpenAI Function Calling to extract laptop details based on user input.
    """
    final_message = [
    ...
    missing_params = []
    if 'GPU intensity' not in function_parameters:
        missing_params.append("GPU intensity")
    if 'Display quality' not in function_parameters:
        missing_params.append("Display quality")
    if 'Portability' not in function_parameters:
        missing_params.append("Portability")
    if 'Multitasking' not in function_parameters:
        missing_params.append("Multitasking")
    if 'Processing speed' not in function_parameters:
        missing_params.append("Processing speed")
    if include_budget and 'Budget' not in function_parameters:
        missing_params.append("Budget")

    if missing_params:
        questions = []
        for param in missing_params:
            if param == "GPU intensity":
                questions.append("What is the desired GPU intensity (low, medium, or high)?")
            elif param == "Display quality":
                questions.append("What is the desired display quality (low, medium, or high)?")
            elif param == "Portability":
                questions.append("What is the desired portability (low, medium, or high)?")
            elif param == "Multitasking":
                questions.append("What is the desired multitasking capability (low, medium, or high)?")
            elif param == "Processing speed":
                questions.append("What is the desired processing speed (low, medium, or high)?")
            elif param == "Budget":
                questions.append("What is your budget?")

        final_message.append({"role": "user", "content": "Please provide the following information:\n" + "\n".join(questions)})
    ...
    processing_speed = function_parameters.get('Processing speed', 'medium')
    ...
    return extract_user_info(function_parameters['GPU intensity'], function_parameters['Display quality'], function_parameters['Portability'], function_parameters['Multitasking'],
                                       processing_speed, budget)

# Returns a dictionary containing the extracted laptop information.
def extract_user_info(GPU_intensity, Display_quality, Portability, Multitasking, Processing_speed, Budget):
    """
    Organizes the extracted user data into a dictionary format.
    """
...
# Compares the user's requirements with the available laptops and returns the top 3 matching laptops.
def compare_laptops_with_user(user_requirements):
    """
    Compares the user requirements against a dataset of available laptops and ranks them based on relevance.
    """
    laptop_df= pd.read_csv('shopAssit_2\laptop_data.csv')
...
# Validates the laptop recommendations.
def recommendation_validation(laptop_recommendation):
    """
    Validates the laptop recommendations based on the score and other criteria.
    """
    data = json.loads(laptop_recommendation)
    return [laptop for laptop in data if laptop['Score'] > 2]

# Initializes the conversation for the recommendation phase.
def initialize_conv_recommendation(products):
    """
    Initializes the conversation for the recommendation phase, based on the available laptops.
    """
    system_message = f"""
...
# Extracts the key features of a laptop from its description and classifies them into low, medium, or high.
def product_map_layer(laptop_description):
    """
    Extracts key features from the laptop description and classifies them into 'low', 'medium', or 'high'.
    """
    delimiter = "#####"
    ...
    try:
        ...
        print(f"Error in mapping laptop features: {e}")
        return None
